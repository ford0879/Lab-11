...
public class GraphicalCalculatorFrame extends JFrame
{
	...
	private final class GraphicalCalculatorPanel extends JPanel implements MouseListener
	{
		...
		private void setupRegions()
		{
			...
			{
				regions[index] = new Rectangle(regions[index-1].x + REGION_INC_X, REGION_START_Y, REGION_WIDTH, REGION_HEIGHT);
			}
		}
		...
		protected void paintComponent(Graphics g)
		{
			...
			{
				g.drawRect(regions[index].x, regions[index].y, regions[index].width, regions[index].height);
			}
		...
		public void mouseClicked(MouseEvent e)
		{
			...
			for(int index = 0; index < regions.length; ++index)
			{
				if(regions[index].contains(e.getPoint()))
				{
		...
		public int evaluate()
		{
			...
			// Adds, subtracts, or multiplies first and second operands
			...
			// Adds, subtracts, or multiplies previous result with third operand
    ...
    public GraphicalCalculatorFrame() {
        ...
        setOperator.addActionListener((e) -> {
        	...
        	String selected = "";
        	if(add.isSelected())
        		selected = add.getActionCommand();
        	
        	else if(subtract.isSelected())
        		selected = subtract.getActionCommand();
        	
        	else if(multiply.isSelected())
        		selected = multiply.getActionCommand();
        	
        	if(gcPanel.setSelectedRegionContents(selected) == true)
           	 errorMessage.setText("");
           	else
